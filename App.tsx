...
const App: React.FC = () => {
  ...
  const [includeAuthorOnCover, setIncludeAuthorOnCover] = useState(true);
  ...
  const [currentBook, setCurrentBook] = useState<Partial<BookHistoryEntry>>({});
  ...
  useEffect(() => {
    try {
      ...
      if (savedBook) {
        ...
        setIncludeAuthorOnCover(data.includeAuthorOnCover ?? true);
        ...
        setFrontCoverPrompt(data.frontCoverPrompt || `A book cover for a ${data.bookType} book titled "${data.title}".`);
        setBackCoverPrompt(data.backCoverPrompt || `The back cover for the book titled "${data.title}", showing a complementary scene or theme.`);
      }
  ...
  const handleGenerateToc = async () => {
    if (!title || !description || !numChapters || +numChapters <= 0) {
      setError('Please provide a valid title, description, and number of chapters.');
      return;
    }
    setIsLoading(true);
  ...
  const handleGenerateBothCovers = async () => {
    ...
    try {
        await Promise.all([
            handleGenerateCover('front'),
            handleGenerateCover('back')
        ]);
    } catch (e) {
        setError(e instanceof Error ? e.message : "Failed to generate one or both covers. The model may have refused a prompt.");
    } finally {
  ...
  const handleChapterTitleChange = (index: number, newTitle: string) => setChapters(prev => prev.map((ch, i) => i === index ? { ...ch, title: newTitle } : ch));
  ...
  const handleGenerateSingleChapter = useCallback(async (chapterIndex: number) => {
    ...
    if (continueFromPrevious && chapterIndex > 0) {
        for (let i = chapterIndex - 1; i >= 0; i--) {
            if (chapters[i].status === 'done' && chapters[i].content) {
                previousChapterContent = chapters[i].content;
                break;
            }
        }
    }
  ...
  const handleGenerateAllChapters = async () => {
    ...
    for (let i = 0; i < currentChapters.length; i++) {
        if (currentChapters[i].status === 'done' && currentChapters[i].content) {
            lastContent = continueFromPrevious ? currentChapters[i].content : '';
            continue;
        }
        ...
        try {
            ...
            setChapters(prev => prev.map((ch, idx) => idx === i ? { ...ch, status: 'pending' } : ch));
            setIsLoading(false);
            return;
        }
    }
  ...
  const handleAssembleBook = async () => {
    ...
    try {
        ...
        setCurrentBook(newHistoryEntry);
    ...
    <div className="bg-white/5 p-6 sm:p-8 rounded-2xl shadow-2xl backdrop-blur-sm border border-white/10">
      ...
      <div className="space-y-4">
         <div className="grid md:grid-cols-2 gap-4">
            ...
            <input type="text" value={subtitle} onChange={(e) => setSubtitle(e.target.value)} placeholder="Subtitle / Tagline (Optional)" className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-4 py-3 focus:ring-2 focus:ring-indigo-500 outline-none transition" />
         </div>
         <div className="relative">
            <input type="text" value={author} onChange={(e) => setAuthor(e.target.value)} placeholder="Author Name(s) (Optional)" className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-4 py-3 focus:ring-2 focus:ring-indigo-500 outline-none transition" />
            <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center">
                <input id="author-on-cover" type="checkbox" checked={includeAuthorOnCover} onChange={(e) => setIncludeAuthorOnCover(e.target.checked)} className="h-4 w-4 rounded bg-gray-700 border-gray-600 text-indigo-600 focus:ring-indigo-500" />
                <label htmlFor="author-on-cover" className="ml-2 text-xs text-gray-400">On Covers</label>
            </div>
         </div>
        <div>
          <label htmlFor="description" className="block text-sm font-medium text-gray-300 mb-2">Book Description</label>
          <div className="relative">
            <textarea
              id="description" value={description} onChange={(e) => setDescription(e.target.value)}
              placeholder="e.g., A space opera about sentient robots who discover the last human..."
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
                ...
                <select value={tone} onChange={e => setTone(e.target.value)} className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-4 py-3 focus:ring-2 focus:ring-indigo-500 outline-none transition">
        ...
        <button onClick={handleGenerateToc} disabled={isLoading || !title || !description || !numChapters || +numChapters <= 0} className="w-full flex items-center justify-center gap-2 text-lg font-bold bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-800/50 disabled:cursor-not-allowed text-white px-6 py-4 rounded-lg transition-transform duration-200 ease-in-out transform hover:scale-105 disabled:transform-none">
    ...
    <div className="bg-white/5 p-6 sm:p-8 rounded-2xl shadow-2xl backdrop-blur-sm border border-white/10">
        ...
        <div className="grid md:grid-cols-2 gap-8">
            <div className="space-y-3">
                ...
                <button onClick={() => handleGenerateCover('front')} disabled={isGeneratingFrontCover || isGeneratingBothCovers} className="w-full flex justify-center items-center gap-2 font-semibold bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-600 px-4 py-2 rounded-lg transition">
            ...
            <div className="space-y-3">
                ...
                <button onClick={() => handleGenerateCover('back')} disabled={isGeneratingBackCover || isGeneratingBothCovers} className="w-full flex justify-center items-center gap-2 font-semibold bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-600 px-4 py-2 rounded-lg transition">
        ...
        <div className="mt-8 border-t border-white/10 pt-6 flex flex-col gap-4">
            <button onClick={handleGenerateBothCovers} disabled={isGeneratingFrontCover || isGeneratingBackCover || isGeneratingBothCovers} className="w-full flex justify-center items-center gap-2 font-bold bg-purple-600 hover:bg-purple-700 disabled:bg-gray-600 px-6 py-3 rounded-lg transition">
                {isGeneratingBothCovers ? <><LoadingSpinner /> Generating Both...</> : 'Generate Both Covers'}
            </button>
            <div className="flex justify-between items-center w-full">
                <button onClick={() => setAppStep('CONFIG')} className="flex items-center gap-2 text-sm bg-gray-600/50 hover:bg-gray-600 text-white font-semibold px-4 py-2 rounded-lg transition">
                    <ChevronLeftIcon className="w-4 h-4" /> Back to Config
                </button>
    ...
    <div className="bg-white/5 p-6 sm:p-8 rounded-2xl shadow-2xl backdrop-blur-sm border border-white/10">
      <div className="flex justify-between items-start sm:items-center border-b border-white/10 pb-4 mb-6 flex-col sm:flex-row gap-2">
        <div>
            ...
            <p className="text-gray-400">Title: <span className="font-semibold text-gray-200">{title}</span></p>
        </div>
          ...
          <li key={index} className="bg-gray-900/50 p-4 rounded-lg border border-white/10 transition-all duration-300">
            <div className="flex justify-between items-start gap-4">
              <div className="flex-grow">
                <div className="flex items-center gap-2">
                    <span className="font-bold text-lg text-gray-100 select-none">{`${index + 1}.`}</span>
                    <input 
                        type="text" 
                        value={chapter.title}
                        onChange={(e) => handleChapterTitleChange(index, e.target.value)}
                        className="w-full bg-transparent font-bold text-lg text-gray-100 focus:bg-gray-800/50 focus:ring-1 focus:ring-indigo-500 rounded px-2 py-1 outline-none transition"
